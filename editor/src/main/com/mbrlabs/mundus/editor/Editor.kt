...
import com.mbrlabs.mundus.editor.plugin.ManagerHolderImpl
...
class Editor : Lwjgl3WindowAdapter(), ApplicationListener,
    ...
    private fun initPluginSystem() {
        ...
        // Setup manager holder for plugins
        pluginManager.getExtensions(ManagerHolderExtension::class.java).forEach {
            try {
                it.managerHolder(ManagerHolderImpl())
            } catch (ex: Exception) {
                Mundus.postEvent(LogEvent(LogType.ERROR, "Exception during passing manager holder! $ex"))
            }
        }
